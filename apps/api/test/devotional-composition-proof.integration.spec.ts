...
if (!databaseUrl) {
  throw new Error('DATABASE_URL is required for the Devotional Composition proof.');
}

const runMarker = `api-devotional-composition-${randomUUID()}`;

const DEVOTIONAL_IMAGE = Buffer.from(
...
describe('P8-M08 Devotional Composition proof', () => {
  ...
  async function createScripture(actor: ActorFixture): Promise<string> {
    ...
    const moduleRef = await Test.createTestingModule({
      imports: [
        AppModule.register({
          ...
          aiProviderKey: 'provider.devotional-composition-proof',
          aiProvider: {
            async generateText(input) {
              providerRequests.push(input);
              return {
                text: DEVOTIONAL_TEMPLE_RESOURCE_TYPE,
                model: 'model.devotional-composition-proof',
                usage: { inputTokens: 24, outputTokens: 3, totalTokens: 27 },
              };
            },
          },
        }),
  ...
  it('composes and publishes a source-aware Devotional Page through shared owner contracts', async () => {
    const creator = await signInAdministrator();
    const publishedScriptureId = await createScripture(creator);
    const draftScriptureId = await createScripture(creator);

    await publishKnowledgeAsActor.execute({
      actingActorId: creator.actorId,
      id: parseResourceId(publishedScriptureId),
    });
    ...
    const suggestion = await request(app.getHttpServer())
      ...
      .send({
        universeKey: DEVOTIONAL_UNIVERSE_KEY,
        request: 'Create a Temple presentation grounded in published Scripture.',
    ...
    expect(providerRequests[0]?.instructions).not.toContain(draftScriptureId);
    await expect(
    ...
    const acceptance = await request(app.getHttpServer())
      ...
      .set('Cookie', creator.cookiePair)
      .expect(201);
    const templeId = acceptance.body.resource.id as string;

    knowledgeIds.add(templeId);
    ...
    const page = await request(app.getHttpServer())
      ...
      .send({
        universeKey: DEVOTIONAL_UNIVERSE_KEY,
        routePath: `/devotional/home-${randomUUID()}`,
    ...
    const block = await request(app.getHttpServer())
      ...
      .send({
        universeKey: DEVOTIONAL_UNIVERSE_KEY,
        text: 'Explore this Temple through its published Scripture source.',
      })
      .expect(201);
    ...
    const media = await request(app.getHttpServer())
      .post('/media/assets')
      .set('Cookie', creator.cookiePair)
      .attach('file', DEVOTIONAL_IMAGE, {
        filename: 'devotional-temple.png',
    ...
    expect(media.body).toMatchObject({
      ...
      technicalMetadata: {
        mimeType: 'image/png',
        sizeBytes: DEVOTIONAL_IMAGE.byteLength,
      },
    });
    ...
    await expect(storage.readObject(persistedAsset.storageReference)).resolves.toEqual(
      new Uint8Array(DEVOTIONAL_IMAGE),
    );

    const orderedItems = [
      { kind: 'BLOCK', id: blockId },
      { kind: 'KNOWLEDGE_RESOURCE', id: templeId },
      { kind: 'KNOWLEDGE_RESOURCE', id: publishedScriptureId },
    ...
    expect(generation).toMatchObject({
      ...
      provider: 'provider.devotional-composition-proof',
      model: 'model.devotional-composition-proof',
      status: 'SUCCEEDED',
      result: { text: DEVOTIONAL_TEMPLE_RESOURCE_TYPE },
      provenance: {
        task: AI_CREATOR_KNOWLEDGE_RESOURCE_CANDIDATE_TASK,
        sourceContext: { universeKey: DEVOTIONAL_UNIVERSE_KEY },
      },
    ...
    expect(sourceContext.universeKey).toBe(DEVOTIONAL_UNIVERSE_KEY);
