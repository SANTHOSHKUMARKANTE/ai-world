...
import type { PublicKnowledgeEntityMedia } from './public-knowledge-entity-api';
...
export function EntityMediaViewer({
  media,
  displayName,
  viewerKind = 'anime',
  onRequestClose,
  ...
  readonly viewerKind?: 'anime' | 'devotional';
  ...
  return (
    <dialog
      ref={dialogRef}
      className="aw-anime-media-viewer aw-entity-media-viewer"
      data-entity-media-viewer="true"
      data-viewer-kind={viewerKind}
      aria-labelledby="aw-entity-media-viewer-title"
      onCancel={(event) => {
        ...
        <div className="aw-anime-media-viewer__surface">
          <header className="aw-anime-media-viewer__header">
            <div>
              ...
              <h2 id="aw-entity-media-viewer-title">{displayName} media</h2>
            </div>
            <button
              ref={closeButtonRef}
              type="button"
          ...
          <div className="aw-anime-media-viewer__frame">
            {media.assetType === 'VIDEO' ? (
              <video
                key={media.assetId}
                data-entity-viewer-video="true"
                data-anime-viewer-video={viewerKind === 'anime' ? 'true' : undefined}
                data-character-viewer-video={viewerKind === 'anime' ? 'true' : undefined}
                data-devotional-viewer-video={viewerKind === 'devotional' ? 'true' : undefined}
