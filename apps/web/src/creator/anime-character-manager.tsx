...
import {
  ANIME_CHARACTER_SECTION_DEFINITIONS,
  animeCharacterSectionOrder,
  findAnimeCharacterSection,
} from '../anime/anime-character-sections';
...
export interface AnimeCharacterManagerProps {
...
function isAnimeCharacterResource(resource: CreatorKnowledgeResource | null): boolean {
  return resource?.universeKey === 'universe.anime' && resource.resourceType === 'anime.character';
}
  ...
  return [...configuration.relations]
    .sort((left, right) => {
      const sectionDifference =
        animeCharacterSectionOrder(left.sectionKey) - animeCharacterSectionOrder(right.sectionKey);
...
export function AnimeCharacterManager({
  knowledgeResourceId,
  initialResource,
  onKnowledgeResourceIdChange,
  onResourceChange,
}: AnimeCharacterManagerProps) {
  const initialCharacterResource = isAnimeCharacterResource(initialResource)
    ? initialResource
    : null;

  const [resource, setResource] = useState<CreatorKnowledgeResource | null>(
    initialCharacterResource,
  );
  ...
  const firstSection = ANIME_CHARACTER_SECTION_DEFINITIONS[0];
  ...
  const canInitialize = isAnimeCharacterResource(resource);
  ...
  function applyConfiguration(configuration: CreatorKnowledgeEntityConfiguration): void {
    if (!isAnimeCharacterResource(configuration.resource)) {
      throw new Error(
        'Anime Character management requires an universe.anime / anime.character Knowledge Resource.',
      );
    }
  ...
  async function loadCharacter(): Promise<void> {
    ...
    if (!id) {
      setErrorMessage('Enter a Character Knowledge Resource ID before loading.');
      return;
    }

    setBusyAction('load-character');
    setStatusMessage(null);
    setErrorMessage(null);
    ...
    try {
      ...
      setStatusMessage(`Loaded Character configuration for ${configuration.displayName}.`);
    } catch (error) {
  ...
  function addFact(): void {
    if (facts.length >= 12) {
      setErrorMessage('A Character can have no more than 12 quick facts.');
      return;
    }
    ...
    setNewFact({ key: 'anime.role', label: 'Role', value: '' });
    setErrorMessage(null);
  }
  ...
  function addRelation(): void {
    ...
    const section =
      findAnimeCharacterSection(newRelation.sectionKey) ?? ANIME_CHARACTER_SECTION_DEFINITIONS[0];
    setNewRelation({
      targetResourceId: '',
  ...
  async function saveCharacter(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();

    const id = knowledgeResourceId.trim();
    if (!id) {
      setErrorMessage('Enter or create an Anime Character Knowledge Resource first.');
      return;
    }

    if (!canInitialize && !loaded) {
      setErrorMessage(
        'Load an existing Anime Character or create an anime.character Resource in this workspace before saving.',
      );
      return;
    }
    ...
    try {
      ...
      applyConfiguration(configuration);
      setStatusMessage(`Saved Character configuration for ${configuration.displayName}.`);
    } catch (error) {
      setErrorMessage(getApiErrorMessage(error));
    } finally {
      setBusyAction(null);
  ...
  async function publishCharacter(): Promise<void> {
    if (!resource || resource.lifecycle !== 'DRAFT') {
      return;
    }

    setBusyAction('publish-character');
    setStatusMessage(null);
    setErrorMessage(null);
    ...
    try {
      ...
      setStatusMessage(`Published Character Knowledge Resource ${published.id}.`);
    } catch (error) {
      setErrorMessage(getApiErrorMessage(error));
    } finally {
      setBusyAction(null);
  ...
  async function archiveCharacter(): Promise<void> {
    if (!resource || resource.lifecycle !== 'PUBLISHED') {
      return;
    }

    setBusyAction('archive-character');
    setStatusMessage(null);
    setErrorMessage(null);
    ...
    try {
      ...
      setStatusMessage(`Archived Character Knowledge Resource ${archived.id}.`);
    } catch (error) {
      setErrorMessage(getApiErrorMessage(error));
    } finally {
      setBusyAction(null);
  ...
  return (
    <section
      aria-labelledby="creator-anime-character-title"
    ...
    >
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          ...
          <h2 id="creator-anime-character-title" className="mt-2 text-2xl font-semibold">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {busyAction === 'load-character' ? 'Loading…' : 'Load Character configuration'}
          </button>
          {knowledgeResourceId.trim() ? (
            <Link
              className={secondaryButtonClassName}
              href={`/creator/characters/${encodeURIComponent(knowledgeResourceId.trim())}/preview`}
            >
              Preview Character
            </Link>
          ) : null}
      ...
      <label className="mt-5 block text-sm font-medium" htmlFor="creator-character-resource-id">
        Character Knowledge Resource ID
        <input
          id="creator-character-resource-id"
      ...
      <form onSubmit={saveCharacter} className="mt-6 space-y-6">
        <fieldset className="grid gap-4 rounded-xl border border-slate-800 p-4 sm:grid-cols-2">
          <legend className="px-2 text-sm font-semibold text-orange-200">Character identity</legend>

          <label className="block text-sm font-medium" htmlFor="creator-character-slug">
            Slug
            <input
              id="creator-character-slug"
              className={inputClassName}
              value={slug}
              disabled={busy}
          ...
          <label className="block text-sm font-medium" htmlFor="creator-character-display-name">
            Display name
            <input
              id="creator-character-display-name"
              className={inputClassName}
              value={displayName}
              disabled={busy}
          ...
          <label className="block text-sm font-medium" htmlFor="creator-character-native-name">
            Native name
            <input
              id="creator-character-native-name"
              className={inputClassName}
              value={nativeName}
              disabled={busy}
          ...
          <label className="block text-sm font-medium" htmlFor="creator-character-alternate-names">
            Alternate names · one per line
            <textarea
              id="creator-character-alternate-names"
          ...
          <label
            ...
            htmlFor="creator-character-summary"
          >
            Short summary
            <textarea
              id="creator-character-summary"
          ...
          <label
            className="block text-sm font-medium sm:col-span-2"
            htmlFor="creator-character-overview"
          >
            Long overview
            <textarea
              id="creator-character-overview"
        ...
        <fieldset className="rounded-xl border border-slate-800 p-4">
          ...
          <ol aria-label="Character quick facts" className="space-y-3">
        ...
        <fieldset className="rounded-xl border border-slate-800 p-4">
          ...
          <ol aria-label="Character relationships" className="space-y-3">
                ...
                >
                  <label className="text-xs font-medium text-slate-400">
                    ...
                    <select
                      ...
                      disabled={busy}
                      onChange={(event) => {
                        const definition = findAnimeCharacterSection(event.target.value);
                        setRelations((current) =>
                    ...
                    >
                      {ANIME_CHARACTER_SECTION_DEFINITIONS.map((section) => (
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-3">
            <label className="text-sm font-medium" htmlFor="creator-new-relation-section">
              ...
              <select
                ...
                onChange={(event) => {
                  const definition =
                    findAnimeCharacterSection(event.target.value) ??
                    ANIME_CHARACTER_SECTION_DEFINITIONS[0];
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {resource?.lifecycle === 'DRAFT' && loaded ? (
            ...
            >
              {busyAction === 'publish-character' ? 'Publishing…' : 'Publish Character'}
            </button>
          ) : null}
          ...
          {resource?.lifecycle === 'PUBLISHED' ? (
            ...
            >
              {busyAction === 'archive-character' ? 'Archiving…' : 'Archive Character'}
            </button>
          ) : null}

          {resource?.lifecycle === 'ARCHIVED' ? (
            <span className="text-sm text-slate-400">
              Archived Character Resources are terminal.
            </span>
          ) : null}
        </div>
      </form>
