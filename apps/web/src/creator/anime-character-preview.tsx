...
function AuthenticatedAnimeCharacterPreview({ resourceId }: { readonly resourceId: string }) {
    ...
    void Promise.all([getCreatorKnowledgeEntity(resourceId), getCreatorKnowledgeMedia(resourceId)])
      .then(([entity, media]) => {
        ...
        if (
          ...
          setState({
            status: 'error',
            message: 'Creator Character preview requires an Anime Character Resource.',
          });
          return;
        }
  ...
  if (state.status === 'loading') {
    return <p className="text-slate-400">Loading Creator-only Character preview…</p>;
  }

  if (state.status === 'error') {
    return (
      <section className="rounded-2xl border border-rose-500/30 bg-rose-500/10 p-6">
        <h1 className="text-2xl font-semibold">Character preview unavailable</h1>
  ...
  const visibleSections = ANIME_CHARACTER_SECTION_KEYS.filter((sectionKey) =>
  ...
  return (
    <article className="space-y-7">
      <header className="rounded-3xl border border-orange-400/25 bg-gradient-to-br from-orange-400/15 via-slate-900 to-slate-950 p-6 sm:p-9">
        <div className="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em]">
          <span className="rounded-full bg-orange-300 px-3 py-1 text-slate-950">
            Creator-only Character preview
          </span>
      ...
      {orderedMedia.length > 0 ? (
        <section aria-labelledby="character-preview-media-title">
          <h2 id="character-preview-media-title" className="text-2xl font-semibold">
            Media placements
          </h2>
      ...
      <section
        ...
        aria-labelledby="character-preview-overview-title"
      >
        <h2 id="character-preview-overview-title" className="text-2xl font-semibold">
      ...
      {visibleSections.map((sectionKey) => {
        const definition = ANIME_CHARACTER_SECTION_DEFINITIONS.find(
        ...
        return (
          <section
            key={sectionKey}
            aria-labelledby={`character-preview-${sectionKey.replaceAll('.', '-')}`}
            className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6"
          >
            <h2
              id={`character-preview-${sectionKey.replaceAll('.', '-')}`}
...
export function AnimeCharacterPreview({ resourceId }: { readonly resourceId: string }) {
  const { state, refreshSession } = useSession();

  switch (state.status) {
    case 'loading':
      return <p className="text-slate-400">Checking Character preview access…</p>;
    case 'anonymous':
      return (
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <h1 className="text-2xl font-semibold">Sign in to preview this Character</h1>
          <p className="mt-3 text-slate-400">
            DRAFT Character configuration is available only through the authorized Creator boundary.
          </p>
          <Link
      ...
      return (
        <section className="rounded-2xl border border-rose-500/30 bg-rose-500/10 p-6">
          <p role="alert" className="text-rose-200">
            Character preview session status is unavailable.
          </p>
          <button
    ...
    case 'authenticated':
      return <AuthenticatedAnimeCharacterPreview resourceId={resourceId} />;
  }
}
