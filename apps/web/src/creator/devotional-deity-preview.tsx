...
import { DEVOTIONAL_DEITY_SECTION_KEYS } from '../devotional/devotional-deity-vocabulary';
import {
...
const devotionalPresentation = resolveWebUniversePresentation('universe.devotional');

function AuthenticatedDevotionalDeityPreview({ resourceId }: { readonly resourceId: string }) {
    ...
    void Promise.all([getCreatorKnowledgeEntity(resourceId), getCreatorKnowledgeMedia(resourceId)])
      .then(([entity, media]) => {
        ...
        if (
          ...
          setState({
            status: 'error',
            message: 'Creator Deity preview requires a Devotional Deity Resource.',
          });
          return;
        }
  ...
  if (state.status === 'loading') {
    return <p className="text-slate-400">Loading Creator-only Deity preview…</p>;
  }

  if (state.status === 'error') {
    return (
      <section className="rounded-2xl border border-rose-500/30 bg-rose-500/10 p-6">
        <h1 className="text-2xl font-semibold">Deity preview unavailable</h1>
  ...
  const visibleSections = DEVOTIONAL_DEITY_SECTION_KEYS.filter((sectionKey) =>
    groupedRelations.has(sectionKey),
  );

  return (
    <article className="space-y-7" data-creator-devotional-deity-preview="true">
      <header className="rounded-3xl border border-amber-400/25 bg-gradient-to-br from-amber-400/15 via-slate-900 to-slate-950 p-6 sm:p-9">
        <div className="flex flex-wrap items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em]">
          ...
          <span className="text-amber-100/70">{entity.resource.lifecycle}</span>
        </div>
      ...
      {orderedMedia.length > 0 ? (
        <section aria-labelledby="deity-preview-media-title">
          <h2 id="deity-preview-media-title" className="text-2xl font-semibold">
            Media placements
          </h2>
      ...
      <section
        ...
        aria-labelledby="deity-preview-overview-title"
      >
        <h2 id="deity-preview-overview-title" className="text-2xl font-semibold">
      ...
      {visibleSections.map((sectionKey) => {
        ...
        return (
          <section
            key={sectionKey}
            aria-labelledby={`deity-preview-${sectionKey.replaceAll('.', '-')}`}
            className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6"
          >
            <h2
              id={`deity-preview-${sectionKey.replaceAll('.', '-')}`}
              className="text-2xl font-semibold"
            >
              {resolveEntitySectionTitle(devotionalPresentation, sectionKey, entity.displayName)}
            </h2>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {entity.resource.lifecycle === 'PUBLISHED' ? (
          <Link
            href={`/devotional/${encodeURIComponent(entity.slug)}`}
...
export function DevotionalDeityPreview({ resourceId }: { readonly resourceId: string }) {
  const { state, refreshSession } = useSession();

  switch (state.status) {
    case 'loading':
      return <p className="text-slate-400">Checking Deity preview access…</p>;
    case 'anonymous':
      return (
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <h1 className="text-2xl font-semibold">Sign in to preview this Deity</h1>
          <p className="mt-3 text-slate-400">
            DRAFT Deity configuration is available only through the authorized Creator boundary.
          </p>
          <Link
            className="mt-5 inline-block rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-slate-950"
            href="/sign-in"
          >
            Sign in
          </Link>
      ...
      return (
        <section className="rounded-2xl border border-rose-500/30 bg-rose-500/10 p-6">
          <p role="alert" className="text-rose-200">
            Deity preview session status is unavailable.
          </p>
          <button
    ...
    case 'authenticated':
      return <AuthenticatedDevotionalDeityPreview resourceId={resourceId} />;
  }
}
