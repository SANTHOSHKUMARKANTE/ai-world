...
import {
  DEVOTIONAL_DEITY_FACT_TEMPLATES,
  DEVOTIONAL_DEITY_SECTION_DEFINITIONS,
  devotionalDeitySectionOrder,
  findDevotionalDeitySection,
} from '../devotional/devotional-deity-vocabulary';
...
export interface DevotionalDeityManagerProps {
...
const inputClassName =
  'mt-2 w-full rounded-xl border border-slate-700 bg-slate-950/80 px-3 py-2.5 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20 disabled:opacity-60';
const primaryButtonClassName =
  'rounded-xl bg-amber-400 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-50';
...
function isDevotionalDeityResource(resource: CreatorKnowledgeResource | null): boolean {
  return (
    resource?.universeKey === 'universe.devotional' && resource.resourceType === 'devotional.deity'
  );
}
  ...
  return [...configuration.relations]
    .sort((left, right) => {
      const sectionDifference =
        devotionalDeitySectionOrder(left.sectionKey) -
        devotionalDeitySectionOrder(right.sectionKey);
...
export function DevotionalDeityManager({
  knowledgeResourceId,
  initialResource,
  onKnowledgeResourceIdChange,
  onResourceChange,
}: DevotionalDeityManagerProps) {
  const initialDeityResource = isDevotionalDeityResource(initialResource) ? initialResource : null;

  const [resource, setResource] = useState<CreatorKnowledgeResource | null>(initialDeityResource);
  ...
  const [newFact, setNewFact] = useState<FactDraft>({
    ...DEVOTIONAL_DEITY_FACT_TEMPLATES[0],
  });
  const firstSection = DEVOTIONAL_DEITY_SECTION_DEFINITIONS[0];
  ...
  const canInitialize = isDevotionalDeityResource(resource);
  ...
  function applyConfiguration(configuration: CreatorKnowledgeEntityConfiguration): void {
    if (!isDevotionalDeityResource(configuration.resource)) {
      throw new Error(
  ...
  async function loadDeity(): Promise<void> {
    const id = knowledgeResourceId.trim();
    if (!id) {
      setErrorMessage('Enter a Deity Knowledge Resource ID before loading.');
      return;
    }

    setBusyAction('load-deity');
    setStatusMessage(null);
    setErrorMessage(null);
    ...
    try {
      ...
      setStatusMessage(`Loaded Deity configuration for ${configuration.displayName}.`);
    } catch (error) {
      setErrorMessage(
  ...
  function addFact(): void {
    if (facts.length >= 12) {
      setErrorMessage('A Deity can have no more than 12 quick facts.');
      return;
    }
    ...
    setNewFact({ ...DEVOTIONAL_DEITY_FACT_TEMPLATES[0] });
    setErrorMessage(null);
  }
  ...
  function addRelation(): void {
    ...
    const section =
      findDevotionalDeitySection(newRelation.sectionKey) ?? DEVOTIONAL_DEITY_SECTION_DEFINITIONS[0];
    setNewRelation({
      targetResourceId: '',
  ...
  async function saveDeity(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();

    const id = knowledgeResourceId.trim();
    if (!id) {
      setErrorMessage('Enter or create a Devotional Deity Knowledge Resource first.');
      return;
    }

    if (!canInitialize && !loaded) {
      setErrorMessage(
        'Load an existing Devotional Deity or create a devotional.deity Resource in this workspace before saving.',
      );
      return;
    }
    ...
    try {
      ...
      applyConfiguration(configuration);
      setStatusMessage(`Saved Deity configuration for ${configuration.displayName}.`);
    } catch (error) {
  ...
  async function publishDeity(): Promise<void> {
    if (!resource || resource.lifecycle !== 'DRAFT') {
      return;
    }

    setBusyAction('publish-deity');
    setStatusMessage(null);
    setErrorMessage(null);
    ...
    try {
      ...
      setStatusMessage(`Published Deity Knowledge Resource ${published.id}.`);
    } catch (error) {
      setErrorMessage(getApiErrorMessage(error));
    } finally {
      setBusyAction(null);
  ...
  async function archiveDeity(): Promise<void> {
    if (!resource || resource.lifecycle !== 'PUBLISHED') {
      return;
    }

    setBusyAction('archive-deity');
    setStatusMessage(null);
    setErrorMessage(null);
    ...
    try {
      ...
      setStatusMessage(`Archived Deity Knowledge Resource ${archived.id}.`);
    } catch (error) {
      setErrorMessage(getApiErrorMessage(error));
    } finally {
      setBusyAction(null);
  ...
  return (
    <section
      aria-labelledby="creator-devotional-deity-title"
      data-creator-devotional-deity-manager="true"
      className="mt-6 rounded-2xl border border-amber-400/25 bg-slate-900/80 p-5 shadow-xl shadow-black/10"
    >
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          ...
          <h2 id="creator-devotional-deity-title" className="mt-2 text-2xl font-semibold">
            Deity profile & relationships
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
            Edit the canonical Entity profile, quick facts, and ordered Devotional relationships.
            Media stays in the shared Knowledge Media manager; this editor writes only the existing
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={() => void loadDeity()}
          >
            {busyAction === 'load-deity' ? 'Loading…' : 'Load Deity configuration'}
          </button>
          {knowledgeResourceId.trim() ? (
            <Link
              className={secondaryButtonClassName}
              href={`/creator/deities/${encodeURIComponent(knowledgeResourceId.trim())}/preview`}
            >
              Preview Deity
            </Link>
          ) : null}
      ...
      <label className="mt-5 block text-sm font-medium" htmlFor="creator-deity-resource-id">
        Deity Knowledge Resource ID
        <input
          id="creator-deity-resource-id"
      ...
      <form onSubmit={saveDeity} className="mt-6 space-y-6">
        <fieldset className="grid gap-4 rounded-xl border border-slate-800 p-4 sm:grid-cols-2">
          <legend className="px-2 text-sm font-semibold text-amber-200">Deity identity</legend>

          <label className="block text-sm font-medium" htmlFor="creator-deity-slug">
            Slug
            <input
              id="creator-deity-slug"
              className={inputClassName}
              value={slug}
              disabled={busy}
          ...
          <label className="block text-sm font-medium" htmlFor="creator-deity-display-name">
            Display name
            <input
              id="creator-deity-display-name"
              className={inputClassName}
              value={displayName}
              disabled={busy}
          ...
          <label className="block text-sm font-medium" htmlFor="creator-deity-native-name">
            Native name
            <input
              id="creator-deity-native-name"
              className={inputClassName}
              value={nativeName}
              disabled={busy}
          ...
          <label className="block text-sm font-medium" htmlFor="creator-deity-alternate-names">
            Alternate names · one per line
            <textarea
              id="creator-deity-alternate-names"
          ...
          <label
            ...
            htmlFor="creator-deity-summary"
          >
            Short summary
            <textarea
              id="creator-deity-summary"
          ...
          <label
            className="block text-sm font-medium sm:col-span-2"
            htmlFor="creator-deity-overview"
          >
            Long overview
            <textarea
              id="creator-deity-overview"
        ...
        <fieldset className="rounded-xl border border-slate-800 p-4">
          <legend className="px-2 text-sm font-semibold text-amber-200">Quick facts</legend>

          <div
            aria-label="Preferred Devotional Deity quick fact templates"
            className="mb-4 rounded-xl border border-amber-400/20 bg-amber-400/5 p-3"
          >
            <p className="text-xs font-semibold uppercase tracking-wider text-amber-200">
              Preferred Devotional fact templates
            </p>
            <p className="mt-1 text-xs leading-5 text-slate-400">
              These are UI suggestions over the existing generic namespaced quick-fact contract.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {DEVOTIONAL_DEITY_FACT_TEMPLATES.map((template) => (
                <button
                  key={template.key}
                  type="button"
                  className={secondaryButtonClassName}
          ...
          <ol aria-label="Deity quick facts" className="space-y-3">
        ...
        <fieldset className="rounded-xl border border-slate-800 p-4">
          ...
          <ol aria-label="Deity relationships" className="space-y-3">
                ...
                >
                  <label className="text-xs font-medium text-slate-400">
                    ...
                    <select
                      ...
                      disabled={busy}
                      onChange={(event) => {
                        const definition = findDevotionalDeitySection(event.target.value);
                        setRelations((current) =>
                    ...
                    >
                      {DEVOTIONAL_DEITY_SECTION_DEFINITIONS.map((section) => (
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setRelations((current) =>
                          current.map((candidate, relationIndex) =>
                            relationIndex === index
                              ? { ...candidate, relationshipType: event.target.value }
                              : candidate,
                          ),
                        )
                      }
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-3">
            <label className="text-sm font-medium" htmlFor="creator-new-relation-section">
              ...
              <select
                ...
                onChange={(event) => {
                  const definition =
                    findDevotionalDeitySection(event.target.value) ??
                    DEVOTIONAL_DEITY_SECTION_DEFINITIONS[0];
            ...
            <label className="text-sm font-medium" htmlFor="creator-new-relation-type">
              ...
              <input
                ...
                className={inputClassName}
                list="creator-devotional-preferred-relation-types"
              ...
              />
              <datalist id="creator-devotional-preferred-relation-types">
                {(findDevotionalDeitySection(newRelation.sectionKey)?.relationshipTypes ?? []).map(
                  (relationshipType) => (
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            disabled={busy || !knowledgeResourceId.trim()}
          >
            {busyAction === 'save-deity'
              ? 'Saving…'
              : loaded
                ? 'Save Deity configuration'
                : 'Initialize Deity configuration'}
          </button>
          ...
          {resource?.lifecycle === 'DRAFT' && loaded ? (
            <button
              ...
              type="button"
              disabled={busy}
              onClick={() => void publishDeity()}
            >
              {busyAction === 'publish-deity' ? 'Publishing…' : 'Publish Deity'}
            </button>
          ) : null}
          ...
          {resource?.lifecycle === 'PUBLISHED' ? (
            <button
              ...
              type="button"
              disabled={busy}
              onClick={() => void archiveDeity()}
            >
              {busyAction === 'archive-deity' ? 'Archiving…' : 'Archive Deity'}
            </button>
          ) : null}

          {resource?.lifecycle === 'ARCHIVED' ? (
            <span className="text-sm text-slate-400">Archived Deity Resources are terminal.</span>
          ) : null}
        </div>
      </form>
    </section>
