...
Phase 7
COMPLETE — tagged phase-7-complete

Phase 8 — Composition / CMS Platform
ACTIVE
...
## PHASE 8 ACTIVATION RECORD

Phase 8 — Composition / CMS Platform is ACTIVE.

The activation occurs only after the final Phase 7 closure commit:

```text
33e04894ea638d6b0c5e66461f144bea9deb9a78
docs(roadmap): close Phase 7
```
...
received its own exact successful GitHub Actions CI:
...
and the annotated completion tag was independently verified:
...
TAG OBJECT
db51a16600b4bb089b93f9cba1a2081105a89401
...
The Phase 8 entry requirements are SATISFIED:

```text
Knowledge
SATISFIED — Phase 4 platform is complete and canonical Knowledge Contracts exist

Media
SATISFIED — Phase 5 platform is complete and canonical Media ownership exists

Identity
SATISFIED — shared Identity / authorization platform is implemented and validated

AI Creator assistance
SATISFIED — Phase 7 is complete, tagged, and validated
```

Phase 8 therefore starts with the minimum next real capability:
...
Canonical P8-M01 scope remains exactly:
...
This activation does not pre-design P8-M02 or later milestones.

The Phase 8 ownership rule remains binding:
...
The following expansion points remain demand-driven gates and are NOT activated
merely because Phase 8 begins:
...
This activation intentionally introduces:

```text
no Page production model

no Block model

no Composition package

no CMS package

no Creator editing UI
...
The next implementation decision must inspect the current repository and build
only the minimum architecture necessary for:
...
## P8-M01 PAGE MODEL CLOSURE RECORD

P8-M01 — Page Model is CLOSED.

The accepted implementation baseline is:

```text
39b62d6d935722500ae3643c290bff64919a2d8f
feat(composition): add Page model
```

Parent:

```text
f5d7f0ecb920df76d14794e91249ea728a9bb2ba
docs(roadmap): activate Phase 8
```

The implementation commit contains exactly the previously validated 16-file
P8-M01 candidate and retains the approved Git object identities for every
implementation file.

Its exact successful GitHub Actions CI is:
...
Canonical P8-M01 ownership is:

```text
Composition / CMS Platform
```

The closed Page model is intentionally narrow:

```text
Page

Universe-scoped identity
...
with route identity constrained by:

```text
UNIQUE (universe_key, route_path)
```

This allows the same route path to exist in different Universes while preventing
duplicate route identity inside one Universe.
...
The PostgreSQL vertical proof verifies:

```text
Page creation persists and reloads a Universe-scoped DRAFT Page;

lookup works by Page ID and Universe + route path;

same-Universe duplicate route identity is rejected;

cross-Universe route reuse is allowed;

Page creation does not mutate canonical Knowledge;

Page creation does not mutate canonical Media.
```

P8-M01 deliberately does NOT implement:
...
Knowledge / Media composition references

Creator Editing UI

Preview

Publishing commands / lifecycle transitions
...
The presence of PUBLISHED and ARCHIVED in the Page lifecycle vocabulary does not
activate publishing behavior. Publication operations remain owned by the later
Phase 8 publishing milestone.

The Phase 8 ownership rule remains unchanged:
...
P8-M02 must be designed from current repository evidence and must not pre-design
P8-M03 Layout / Composition or later demand-driven gates.
...
## P8-M02 BLOCK MODEL CLOSURE RECORD

P8-M02 — Block Model is CLOSED.

The accepted implementation baseline is:

```text
f47606628b1ba0c3d34362057202ada60efff529
feat(composition): add Block model
```

Parent:

```text
6afc37c73ef98d08c46d1d96a153014b69989e0f
docs(roadmap): close P8-M01
```

The implementation commit contains the validated 13-file P8-M02 candidate.
...
Canonical P8-M02 ownership is:

```text
Composition / CMS Platform
```

The closed Block model is intentionally narrow:
...
Text Block content is validated as:
...
no embedded null character at the Domain boundary.
```
...
using explicit relational columns rather than a generic JSON payload.

PostgreSQL constrains:

```text
the supported initial Block type;

Text Block content length;

non-whitespace Text Block content.
```

P8-M02 introduced two append-only migrations:

```text
20260821183000_composition_block_model

20260821184500_composition_block_text_constraint_fix
```

The second migration corrects the initial PostgreSQL text constraint by removing
the invalid `chr(0)` expression. PostgreSQL rejects embedded null bytes before a
table constraint is evaluated, while the Domain boundary retains its explicit
...
```text
an independent typed Text Block persists and reloads;

typed Blocks can be created independently in structurally different Universes;

unsupported Block types are rejected at the persistence boundary;

Block creation does not create or mutate a Page;

Block creation does not create or mutate canonical Knowledge.
```

P8-M02 deliberately does NOT implement:

```text
Page-to-Block attachment;

Block ordering or layout;

Knowledge composition references;

Media composition references;

recursive Block composition;

additional speculative Block types;
...
P8-M03 must compose the existing Page and Block semantics from current repository
evidence and may introduce Knowledge or Media references only through their public
owner contracts. It must not pre-design P8-M04 Creator Editing UI or later
milestones.
...
## P8-M03 LAYOUT / COMPOSITION CLOSURE RECORD

P8-M03 — Layout / Composition is CLOSED.

The accepted implementation baseline is:

```text
a8e6eb2765c844c3110b23711aa218b269b90027
feat(composition): add page layout composition
```

Parent:

```text
01bfe2cb6666374070ba001852245b0385c8897b
test(composition): isolate page integration proof
```

The implementation commit contains the validated 13-file P8-M03 candidate.
...
Canonical P8-M03 ownership is:

```text
Composition / CMS Platform
```

The closed Layout / Composition model is intentionally narrow:

```text
PageComposition
    one ordered composition per existing Page

PageCompositionItem
    zero-based position
    exactly one typed canonical reference

BLOCK
    Composition-owned Block reference

KNOWLEDGE_RESOURCE
    Knowledge-owned Resource reference

MEDIA_ASSET
    Media-owned Asset reference
```

Composition resolves canonical foreign references through public owner contracts:
...
The Page, Block, and Knowledge Resource must belong to the same Universe.
Media remains globally reusable through the Media-owned active Asset resolver.

All references are resolved before replacement persistence begins, so a rejected
reference cannot persist a partial Page composition.

The public Composition surface now provides:
...
typed Page composition contracts
```

Persistence is owned by:

```text
composition_page_items
```

using explicit relational columns rather than generic JSON layout state.

PostgreSQL constrains:

```text
unique ordered position inside each Page;

non-negative positions;

exactly one reference column for the declared item kind;

foreign-key integrity for Page, Block, Knowledge Resource, and Media Asset;

Page-owned item deletion through Page cascade;

referenced canonical content retention through restricted deletion.
```

P8-M03 introduced one append-only migration:

```text
20260821213000_composition_page_layout
```
...
```text
ordered Block, Knowledge, and Media references persist and reload;

composition does not mutate the referenced canonical owner records;

one Block can be reused across multiple Pages without ownership transfer;

cross-Universe canonical references are rejected before persistence;

owner rejection cannot persist a partial Page composition.
```

P8-M03 deliberately does NOT implement:

```text
standalone Layout templates;

named slots or regions;

responsive layout rules;

recursive or nested composition;

arbitrary JSON layout state;
...
P8-M04 must expose structured creator editing over current public owner contracts
and must not bypass Knowledge, Media, Page, or Block ownership. It must not
pre-design P8-M05 Preview or later demand-driven gates.
...
## P8-M04 CREATOR EDITING UI CLOSURE RECORD

P8-M04 — Creator Editing UI is CLOSED.

The accepted implementation baseline is:

```text
d0c0280ebd511726823ace019b55df67a131cb9f
feat(creator): add structured editing workspace
```

Parent:

```text
6d4ca37954436f3d79690e437c056e96fe2f3954
docs(roadmap): close P8-M03
```

The implementation commit contains the validated 19-file P8-M04 candidate.
...
Canonical ownership remains distributed across the existing owners:

```text
Composition / CMS Platform
    Page semantics
    Block semantics
    Page composition semantics
    composition editing authorization

Knowledge Platform
    canonical Knowledge Resource creation and validation

Media Platform
    canonical Asset upload, storage, lifecycle, and validation
...
Web Application
    authenticated creator delivery and owner-contract orchestration

API Application
    authenticated REST delivery and runtime composition wiring
```

The closed Creator Editing UI is intentionally structured around current public
owner contracts:
...
Active Universe
    shared scope for new Page, Block, and Knowledge records

Page editor
    create a canonical DRAFT Page
    load an existing Page and its ordered composition

Knowledge editor
    create a canonical Knowledge DRAFT through the Knowledge API

Media editor
    upload a canonical PNG or JPEG Asset through the Media API

Block editor
    create an independent reusable Text Block

Composition editor
    add typed Block, Knowledge Resource, and Media Asset references
    reorder references
    remove references
    persist and reload exact Page composition order
```

The Web Application does not import Prisma or mutate canonical persistence.
Its mutation flow remains:

```text
Web
    → API
    → canonical owning Platform operation
```

The Composition delivery surface added for this milestone is:

```text
POST /composition/pages

GET /composition/pages/:id

POST /composition/blocks/text

GET /composition/blocks/:id

PUT /composition/pages/:id/composition

GET /composition/pages/:id/composition
```

Transport request parsing is strict and separate from canonical Domain
validation. Responses expose explicit delivery DTOs rather than Prisma types.
The Web boundary validates unknown JSON before accepting it as creator state.

P8-M04 introduced the owner-defined Permission:

```text
composition.edit
```

The initial grant is intentionally narrow:
...
Every new Composition creator route requires:

```text
valid opaque Session cookie;

resolved acting Actor;

successful composition.edit Permission evaluation.
```

Authorization executes before transport and canonical input validation, so an
unauthorized Actor cannot use creator routes as an input-validation oracle.
Knowledge creation and Media upload continue to enforce their own owner-defined
Permissions independently.

P8-M04 introduced one append-only migration:

```text
20260821233000_composition_edit_authorization
```
...
The focused authorization and PostgreSQL/API proof verifies:

```text
an Actor with composition.edit is allowed;

an Actor without composition.edit is denied;

a Session is required before Composition creation;

authorization denial precedes malformed-request validation;

an Administrator can create and reload a Page and Text Block;

ordered Block, Knowledge, and Media references persist and reload;

malformed creator requests return safe validation semantics;

cross-Universe references are rejected without partial persistence.
```

The Web and Chromium proof verifies:

```text
anonymous visitors receive an authenticated-entry state;

Page and Block creation use the API boundary;

Knowledge creation uses the existing Knowledge owner route;

Media upload uses the existing Media owner route;

typed references can be added and reordered;

the exact ordered Page composition is submitted through the Composition API.
```

The roadmap phrase `taxonomy/relationships where appropriate` was evaluated
against current implemented demand.

Taxonomy and Relationships remain deferred because P8-M04 introduces no real
classification consumer and no canonical Resource-to-Resource relationship
consumer. Ordered Page composition is represented by the Composition-owned
typed relational model closed in P8-M03; it is not a reason to introduce generic
Kernel Taxonomy or Relationship semantics.

P8-M04 deliberately does NOT implement:

```text
Preview;

publication commands or lifecycle transitions;

Page update semantics beyond the existing public owner contract;

Block update semantics beyond the existing public owner contract;

taxonomy management UI;

relationship management UI;

arbitrary rich-text or JSON layout editing;

nested or responsive layout semantics;
...
P8-M05 must provide controlled draft preview over the current Page, Block, and
typed Page composition contracts. It must not pre-design P8-M06 Publishing or
activate Workflow, Policy, Localization, or Versioning without their accepted
demand gates.
...
## P8-M05 PREVIEW CLOSURE RECORD

P8-M05 — Preview is CLOSED.

The accepted implementation baseline is:

```text
e03ce3bd1f98c47c3cc2ca500ebb0aee85c8a366
feat(composition): add controlled draft preview
```

Parent:

```text
31f5db4ee8aa390186a6322e9f25f4ae07ba75a4
docs(roadmap): close P8-M04
```

The implementation commit contains the validated 18-file P8-M05 candidate.
...
The closed Preview boundary remains Composition-owned and read-only:

```text
Composition / CMS Platform
    preview authorization semantics
    saved Page preview projection
    ordered typed preview item semantics

Knowledge Platform
    canonical Knowledge Resource identity, type, lifecycle, and Universe scope

Media Platform
    canonical ACTIVE Asset reference validation and content delivery
...
Web Application
    authenticated draft-preview presentation

API Application
    authenticated REST delivery and runtime composition wiring
```

Preview is derived from the current persisted canonical state. P8-M05 does not
introduce a second source of truth or separate preview persistence:

```text
saved Page
    + saved ordered Page composition references
    + owner-resolved Block, Knowledge Resource, and Media Asset state
    → read-only PagePreview projection
```

The Composition-owned preview contract resolves the exact persisted order into
the following typed items:
...
KNOWLEDGE_RESOURCE
    canonical Knowledge Resource ID
    Resource type
    lifecycle

MEDIA_ASSET
    canonical ACTIVE Asset ID
```

Block and Knowledge references are revalidated against the Page Universe while
the preview is resolved. Media availability is revalidated through the Media
public reference contract. Missing, inactive, or inconsistent owner state does
not become a partially resolved preview.

The controlled delivery surface added for this milestone is:

```text
GET /composition/pages/:id/preview
```

The endpoint returns an explicit transport DTO and does not expose Prisma
models. A missing Page returns safe not-found semantics.

P8-M05 introduced the owner-defined Permission:
...
Every draft-preview request requires:
...
successful composition.preview Permission evaluation.
```

Authorization executes before identifier and canonical validation, so an
unauthorized Actor cannot use the preview route as an identifier-validation
oracle.

P8-M05 introduced one append-only migration:

```text
20260822003000_composition_preview_authorization
```
...
The Web preview surface is:

```text
/creator/preview/:id
```

The Creator workspace links to the preview for the active Page and explicitly
states that preview reads the last saved composition. The preview surface:

```text
requires an authenticated Session before requesting draft content;

labels the experience as DRAFT preview;

shows Page title, route, Universe, and lifecycle;

renders Text Blocks, Knowledge Resource cards, and Media Assets in saved order;

provides loading, anonymous, session-error, API-error, and empty-composition states.
```

The focused Composition and PostgreSQL/API proof verifies:

```text
an Actor with composition.preview is allowed;

an Actor without composition.preview is denied;

Session and Permission checks precede invalid identifier validation;

Block, Knowledge, and Media references resolve in exact saved order;

cross-Universe preview state is rejected;

a missing Page returns safe not-found semantics;

an Administrator can retrieve the complete owner-resolved draft projection.
```

The Web and Chromium proof verifies:

```text
anonymous visitors do not trigger a draft-content request;

authenticated preview JSON is validated at the Web boundary;

typed preview items render in canonical order;

Media content uses the existing Media delivery route;

the production route manifest includes /creator/preview/[id].
```
...
typecheck — 38/38 tasks;

unit tests — 34/34 tasks;

focused Creator Composition API integration — 7/7 tests;

production build — 21/21 tasks;

Chromium draft-preview E2E;

architecture check — 579 modules and 1775 dependencies with no violations;

database migration status — 25 migrations, schema up to date.
```

P8-M05 deliberately does NOT implement:

```text
unsaved in-memory editor preview;

public or tokenized preview links;

publication commands or lifecycle transitions;

Page or Block update semantics beyond current public owner contracts;
...
P8-M06 must add owner-controlled composition publication lifecycle over the
current Page and typed composition contracts. It must not activate Workflow,
Policy, Localization, or Versioning without their accepted demand gates.
...
## P8-M06 PUBLISHING CLOSURE RECORD

P8-M06 — Publishing is CLOSED.
...
```text
c4db7212a9a98082312d101602f7377a73739b33
docs(roadmap): close P8-M05
```

The implementation commit contains the validated 22-file P8-M06 candidate.
...
The closed publishing boundary remains Composition-owned:

```text
Composition / CMS Platform
    Page lifecycle semantics
    Page publication and archival commands
    composition mutation lock after publication
    owner-defined publishing authorization semantics

Knowledge Platform
    canonical Knowledge Resource identity and lifecycle

Media Platform
    canonical Asset identity and lifecycle
...
Web Application
    authenticated publication controls and lifecycle feedback

API Application
    authenticated REST delivery and runtime composition wiring
```

P8-M06 activates the Page lifecycle vocabulary introduced in P8-M01 through the
following direct Composition-owned transition graph:
...
The transition graph is intentionally one-way:
...
Publishing changes the Composition-owned Page lifecycle only. It does not
transfer ownership of or mutate any referenced canonical Resource:

```text
published Page
    retains ordered typed references to Blocks, Knowledge Resources, and Media Assets
...
Block identity and content
    remain Composition Block-owned
```

Publishing a Page does not implicitly publish a referenced Knowledge Resource,
reactivate a Media Asset, or create a second canonical representation of any
referenced Resource.

The lifecycle persistence contract performs conditional transitions against the
expected current lifecycle. A failed conditional transition is resolved into
safe not-found or lifecycle-conflict semantics rather than silently succeeding.

Composition editing is now restricted to DRAFT Pages at both the Domain and
persistence boundaries:

```text
DRAFT Page
    ordered composition replacement allowed

PUBLISHED Page
    ordered composition replacement rejected

ARCHIVED Page
    ordered composition replacement rejected
```

The Prisma composition replacement transaction conditionally updates the DRAFT
Page row before replacing its ordered items. Publication and editing therefore
coordinate through the same Page row, preventing a concurrent edit from writing
after publication has won the lifecycle transition.

The controlled delivery surfaces added for this milestone are:

```text
POST /composition/pages/:id/publish

POST /composition/pages/:id/archive
```

Both endpoints return explicit creator Page DTOs and do not expose Prisma
models. Missing Pages return safe not-found semantics. Invalid or repeated
transitions return conflict semantics.

P8-M06 introduced the owner-defined Permissions:
...
The initial grants are intentionally narrow:
...
Every publication or archival request requires:
...
successful operation-specific Permission evaluation.
```

Authorization executes before identifier and canonical validation, so an
unauthorized Actor cannot use publishing routes as identifier-validation or
Page-existence oracles.

P8-M06 introduced one append-only migration:

```text
20260822113000_composition_publication_authorization
```
...
The Creator workspace now exposes lifecycle-specific controls:
...
PUBLISHED
    Archive Page
    composition controls disabled

ARCHIVED
    terminal read-only state
```

Unsaved local composition changes make the Publish Page action unavailable and
the workspace instructs the creator to save first. Changing the active Page ID
clears previously loaded Page state so publication cannot accidentally target a
different identifier while showing stale lifecycle information.
...
PUBLISHED transitions to ARCHIVED;

repeat and reverse transitions are rejected;

missing Pages return safe not-found semantics;

publish and archive use distinct Permissions;

unauthorized requests are denied before invalid identifier validation;

published Page composition cannot be replaced;

conditional lifecycle transitions persist through PostgreSQL;

an Administrator can complete the supported lifecycle through the API.
```

The Web and Chromium proof verifies:

```text
creator API publication JSON is validated at the Web boundary;

a DRAFT Page exposes Publish Page;

a PUBLISHED Page exposes Archive Page;

composition editing becomes disabled after publication;

an ARCHIVED Page renders as terminal and read-only;

the existing controlled saved-preview flow remains operational.
```

The exact GitHub Actions Validate job completed every required repository step
successfully:
...
Focused milestone validation also completed successfully:

```text
Composition unit tests — 28/28;

Composition PostgreSQL integration tests — 8/8;

Creator Composition API integration tests — 8/8;

Web unit tests — 34/34;

Chromium creator workspace E2E — 2/2;
...
P8-M06 deliberately does NOT implement:

```text
unpublish or restore transitions;

republishing archived Pages;

scheduled publication;

review, approval, assignment, or rejection states;

Universe-specific publication branching;

implicit publication of referenced Knowledge Resources;

public Page delivery or routing beyond the current controlled preview;
...
The next required roadmap decision is:
...
The Workflow Gate must evaluate current editorial requirements against the
accepted demand conditions in section 200. It must not introduce Workflow Kernel
for hypothetical review, approval, assignment, rejection, or scheduling needs.
...
## PHASE 8 POLICY GATE DECISION RECORD

The Phase 8 Policy Gate is CLOSED.
...
```text
dfb3419f6be9e3b87eb2d94c435ff7d2ee028981
docs(roadmap): close Phase 8 Workflow gate
```

Publishing implementation baseline:
...
### Current owner-local publication rules

Composition currently owns one stable Page lifecycle for every Universe:
...
The lifecycle rules are direct and uniform:

```text
DRAFT Page
    may be published by an authorized Actor

PUBLISHED Page
    may be archived by an authorized Actor
    may not be republished
    may not have composition replaced

ARCHIVED Page
    is terminal
    may not be republished or re-archived
    may not have composition replaced
```

These rules have clear Composition ownership and do not vary through external
policy configuration.

### Cross-context configurability review

Current implemented publication-rule variation is:

```text
by Universe
NONE

by Page or referenced Resource identity
NONE

by region
NONE

by Actor attributes beyond Role/Permission authorization
NONE

by content status beyond the canonical Page lifecycle transition graph
NONE

by locale
NONE

by configured moderation state
NONE

by configured source or citation requirements
NONE
```

The Page `universeKey` establishes canonical scope and route identity. Same-
Universe validation for Block and Knowledge references protects ownership and
consistency. Neither behavior is a configurable publication policy.

Devotional and Anime do not provide separate Page publication requirements,
transition graphs, moderation rules, visibility rules, regional restrictions,
or publication-policy configuration.

### Authorization is not Policy proof

Composition defines stable owner-side Permission vocabulary:
...
Identity & Access owns Role and Permission representation and evaluation.
Composition owns the meaning of its actions and requests evaluation of the
corresponding Permission.

Actor permission assignments can differ, but that is expected authorization
state. It does not establish a generic business Policy representation or a
configurable publication-rule engine.

The existing source file named:

```text
composition-authorization-policy.ts
```

contains stable namespaced Permission keys. The word `policy` in that local file
name does not represent or activate Policy Kernel.
...
generic Policy Rule abstraction
NONE
...
Universe publication-policy configuration
NONE
...
No source package, database schema, migration, API route, UI state, dependency,
or infrastructure was introduced for Policy.
...
```text
Composition / CMS Platform
    owns Page lifecycle, publication action meaning, and canonical Page mutation

Knowledge Platform
    owns Knowledge Resource lifecycle and canonical Knowledge mutation

Media Platform
    owns Asset lifecycle and canonical Media mutation

Identity & Access Platform
    owns authentication and authorization representation/evaluation

Universe or defining capability
    owns domain-specific business Policy configuration when real variation exists

Policy Kernel
    owns shared Policy representation/evaluation mechanics when proven necessary
```

A future Policy Kernel must not absorb Composition, Knowledge, Media, Universe,
or Identity business meaning. The defining consumer retains business-rule
meaning, and the target owner remains responsible for canonical mutation.

### Decision rationale

The current platform contains rules, but the existence of rules is not evidence
for a shared Policy Kernel.

The implemented Page publication rules are:
...
directly encoded by Composition;

not cross-context configurable;

not reused through shared Policy representation or evaluation.
```

Adding Policy now would require premature choices for:
...
evaluation input and result contracts;
...
Universe override semantics;

regional and locale interaction.
```
...
The Phase 8 Policy Gate therefore follows the canonical fallback:
...
```text
defer reusable Policy.
```

### Future activation gate

Policy is not rejected permanently. Re-evaluate when an implemented product
slice demonstrates both:

```text
a business rule must vary through configuration
across Universe, context, region, product, or Resource boundaries

AND

multiple consumers need shared representation and evaluation mechanics.
```

Potential future evidence includes real configurable variation in:
...
content-status dependent behavior;

cross-Universe rule reuse and precedence.
```
...
P8-M07 must integrate existing AI / Creator assistance through its public
contracts and preserve canonical owner validation and acceptance boundaries. It
must not import provider SDKs into Composition or Universe code, treat AI output
as canonical automatically, or activate Policy, Workflow, Localization, or
Versioning without new accepted demand.
...
## P8-M07 AI-ASSISTED CMS CLOSURE RECORD

P8-M07 — AI-Assisted CMS is CLOSED.

The accepted implementation baseline is:

```text
44ada56d3eb439e86cf580dad9f403cad091cabb
feat(composition): add AI-assisted CMS
```

Parent:

```text
14b5ebca907b1871bb464d2261aa00b33087f0c5
docs(roadmap): close Phase 8 Policy gate
```

The implementation commit contains the validated 18-file P8-M07 candidate.
...
The closed AI-assisted CMS boundary remains explicitly multi-owner:

```text
Composition / CMS Platform
    creator assistance orchestration
    review boundary before canonical acceptance
    acceptance Actor, task, and Universe-scope enforcement

AI / Creator Platform
    authorized Generation execution
    normalized Generation state, result, provenance, and usage
    typed Knowledge Resource candidate interpretation
    explicit canonical acceptance contract

Knowledge Platform
    canonical Knowledge Resource validation, mutation, and lifecycle
...
API Application
    authenticated REST delivery
    controlled provider-adapter composition

Web Application
    creator request, review, and explicit acceptance experience
```

P8-M07 integrates the existing Phase 7 public contracts rather than creating a
parallel AI execution architecture:
...
Composition depends only on the deliberate public surface of:

```text
@ai-world/platform-ai-creator
```

It does not import AI / Creator persistence internals or any Provider SDK.

The supported suggestion flow is:

```text
authenticated creator request
    → composition.edit authorization
    → typed transport and canonical input validation
    → authorized AI context resolution
    → ai.generate authorization
    → provider-neutral Generation
    → persisted AI-owned result and provenance
    → typed Knowledge Resource candidate
    → non-canonical creator review state
```

A successful suggestion response is explicit about its status:
...
No Knowledge Resource is created by suggestion execution alone.

Canonical mutation requires a second, explicit acceptance request:

```text
creator selects Accept as Knowledge draft
    → valid Session
    → composition.edit authorization
    → persisted Generation lookup
    → same-Actor verification
    → AI Creator Knowledge-candidate task verification
    → Universe derived from persisted Generation provenance
    → ReviewAndAcceptGenerationAsKnowledgeResource
    → knowledge.resource.create authorization
    → Knowledge-owned validation and DRAFT creation
```

The client does not supply acceptance Universe scope. Composition derives it
from the persisted AI / Creator source context so a client cannot substitute a
different Universe during acceptance.

Composition also rejects acceptance when the Generation:
...
was produced by another AI task;

does not retain an authorized Universe source context;

is not in a successful state;

does not have a persisted result.
```

Those checks occur before Knowledge-owned canonical mutation.
...
The suggestion route accepts bounded provider-neutral input for:
...
published Knowledge context query;

optional context Resource types;

optional bounded context limit.
```

The API transport exposes normalized AI World identifiers and candidate data.
It does not expose Provider SDK request, response, error, or identifier types.

Authorization precedes transport and canonical validation. An anonymous or
unauthorized Actor therefore cannot use the assistance routes as validation,
Generation-existence, or candidate-existence oracles.

P8-M07 reuses the existing owner-defined Permissions:
...
It introduces no new role or Permission vocabulary.

Provider selection remains at the controlled API composition root:
...
test/application override
    → injected AiProviderPort

no configured Provider
    → API remains bootable
    → attempted Generation fails through the existing AI-owned failure path
```

The OpenAI SDK remains isolated inside the accepted AI / Creator adapter. No
Provider SDK import was added to Composition, API transport controllers, Web,
or Universe code.

The Creator workspace now presents an explicit two-stage experience:

```text
Generate AI suggestion
    → render Non-canonical suggestion
    → show proposed typed Resource type
    → add nothing to the canonical reference library

Accept as Knowledge draft
    → create Knowledge-owned DRAFT
    → add only the accepted canonical Resource ID to the creator library
```

Changing the active Universe clears any pending suggestion so a candidate from
one Universe cannot remain displayed under another active Universe selection.
...
no new canonical AI model;

no new canonical Knowledge model;

no Provider SDK dependency outside the existing AI / Creator adapter boundary.
```
...
The focused Composition proof verifies:

```text
suggestion delegates through AI / Creator public contracts;

suggestion does not call canonical acceptance;

acceptance Universe is derived from persisted Generation provenance;

cross-Actor acceptance is rejected;

non-candidate AI tasks are rejected;

missing source scope is rejected;

rejected candidates do not reach canonical owner mutation.
```

The PostgreSQL/API proof verifies:

```text
Session and composition authorization execute before malformed-request validation;

authorized published Knowledge context reaches the Provider Port;

Generation result, provenance, model, Provider key, and usage are persisted;

suggestion creates zero canonical Knowledge Resources;

another Administrator cannot accept the creator's candidate;

the originating creator can explicitly accept the candidate;

accepted canonical output is a Knowledge-owned DRAFT.
```

The Web and Chromium proof verifies:

```text
creator request and context query use the Web-to-API boundary;

the suggestion is visibly identified as non-canonical;

no Knowledge library item exists before acceptance;

explicit acceptance removes the pending suggestion;

the accepted canonical Knowledge reference enters the creator library.
```
...
```text
Composition unit tests — 33/33;

Creator AI assistance PostgreSQL/API integration tests — 2/2;

Creator workspace Web tests — 4/4;

focused Chromium AI review/acceptance E2E — 1/1;

API and Web production builds;

architecture scan — 610 modules and 1892 dependencies, no violations;
...
P8-M07 deliberately does NOT implement:

```text
automatic canonical acceptance of AI output;

client-selected acceptance Universe scope;

cross-Actor candidate acceptance;

provider SDK access from Composition, Web, API transport, or Universe code;

AI-written Page, Block, layout, Knowledge, or Media state outside owner Contracts;

multimodal Generation;

multiple Providers or Provider routing;

semantic or vector Search;

long-running Generation or Queue infrastructure;

Universe-specific branching in shared core;
...
```text
AI / Creator owns Generation and original normalized output.

Knowledge owns accepted canonical Knowledge state.

Composition owns creator review and Page composition semantics.
```
...
P8-M08 must prove the shared Page, Block, typed composition, preview,
publication, and AI-assisted creator contracts through Devotional configuration
and public owner boundaries. It must not introduce named-Universe branching in
shared core, bypass Knowledge or Media ownership, or pre-advance the Anime proof
scheduled for P8-M09.
...
## P8-M08 DEVOTIONAL COMPOSITION PROOF CLOSURE RECORD

P8-M08 — Devotional Composition Proof is CLOSED.

The accepted implementation baseline is:

```text
7118216dc89959d01031c0365afa1c70181197f8
test(composition): prove Devotional composition
```

Parent:

```text
25c15e70994b3595777f39f6409f263a8ec3eb7e
docs(roadmap): close P8-M07
```

Implementation scope:

```text
apps/api/test/devotional-composition-proof.integration.spec.ts
```

Committed proof blob:

```text
87151d13a7b68eff43a61685c871391ee5172439
```
...
Devotional Creator
        ↓
universe.devotional home Page
        ├── typed text Block
        ├── AI-assisted devotional.temple Knowledge reference
        ├── PUBLISHED devotional.scripture Knowledge source
        └── ACTIVE Media Asset
        ↓
...
The proof uses the canonical Devotional definition exported by:
...
No Devotional key or Resource type is duplicated inside shared Composition,
Knowledge, Media, Identity & Access, AI / Creator, or Foundation production
code.
...
Knowledge
    Scripture and accepted Temple canonical state and lifecycle
...
Devotional Universe
    Universe key and closed Devotional Resource-type definition
```

The Scripture/source-aware path deliberately creates both a PUBLISHED and a
DRAFT `devotional.scripture` Resource.

Only the PUBLISHED Scripture is admitted to authorized AI context and persisted
Generation provenance. The DRAFT Scripture is excluded.
...
provider.devotional-composition-proof

model.devotional-composition-proof
```

The suggested `devotional.temple` remains non-canonical after Generation.

Canonical Knowledge mutation occurs only after explicit creator acceptance and
delegates to the Knowledge owner. The accepted Temple begins in the canonical
Knowledge `DRAFT` lifecycle.

The Devotional Page composition resolves in the persisted order:
...
Temple Knowledge Resource
DRAFT

Scripture Knowledge Resource
PUBLISHED
...
Devotional Composition integration proof — 1/1;

neighboring Composition and AI Creator regression set — 11/11 across 3 files;

Creator workspace Chromium proof — 3/3 scenarios passed;

focused architecture scan — 205 modules and 472 dependencies, no violations;
...
P8-M08 proves the shared Composition / CMS contracts through Devotional
configuration and public owner boundaries. It does not create a Devotional fork
of Composition and does not treat earlier generic tests as sufficient evidence.
...
P8-M09 must independently demonstrate reuse through Anime configuration. It
must not be treated as satisfied merely because the Devotional proof passed.

P8-M09 remains blocked until this roadmap-only P8-M08 closure commit itself
completes exact green `push/main` CI.
...
## PHASE 8 RICH EDITOR GATE DECISION RECORD

The Phase 8 Rich Editor Gate is CLOSED.

Decision:

```text
RICH EDITOR TECHNOLOGY
DEFERRED
```
...
### Current editing-model evidence

Composition currently owns one deliberately small typed Block model:
...
The canonical content validation requires non-empty, non-whitespace text and
rejects null characters. It does not define rich-text document nodes, marks,
inline objects, or editor-specific serialized state.

The Creator workspace provides one controlled plain-text input:
...
The resulting text crosses the Web-to-API boundary and is validated and owned
by Composition before it becomes a canonical Text Block.

Page structure is already represented independently through the P8-M03 typed
composition model:

```text
Page
    ordered references to
        Block
        Knowledge Resource
        Media Asset
```

Moving those typed references establishes Page order. It is not arbitrary JSON
layout editing and does not require a rich-text editor framework.

### Implemented product-demand review

The completed creator slices require:

```text
plain Text Block creation
YES

ordered typed Page composition
YES
...
AI-assisted Knowledge candidate review
YES
```

The following rich-editing requirements are absent:

```text
inline formatting marks
NONE

headings inside one rich document
NONE
...
copy/paste normalization rules
NONE
...
undo/redo persistence semantics
NONE
```

Devotional and Anime both compose distinct Pages through the same plain Text
Block and typed-reference model. Neither proof introduced a different rich
content model or Universe-specific editor behavior.

### Canonical selection-criteria review

The accepted Technology Strategy says future editor selection must consider:
...
The current product does not yet provide concrete requirements that can choose
among tradeoffs for those criteria.

Selecting technology now would require speculative decisions for:

```text
canonical rich document schema;

Block type expansion;

allowed nodes and marks;

serialization and migration compatibility;

sanitization and rendering security;

copy/paste normalization;

extension ownership;

accessibility commands and keyboard behavior;

collaboration and conflict semantics;

server-side validation;

preview and publication rendering compatibility;

AI-generated rich document validation.
```

There is no implemented consumer that can validate those choices.

### Current rich-editor materialization

```text
rich editor framework dependency
NONE

rich document Domain Model
NONE

editor-specific Block type
NONE

editor-state persistence
NONE

rich document serializer
NONE
...
rich editor API route
NONE

Universe-specific editor
NONE
```

No source package, database schema, migration, API route, dependency, or
infrastructure is introduced by this decision.
...
```text
Composition / CMS Platform
    owns canonical Block types, content validation, Page composition, preview,
    and publication semantics

Web Application
    owns creator interaction and editor rendering

Knowledge Platform
    owns canonical Knowledge Resource state

Media Platform
    owns canonical Asset state

Universe definitions
    configure domain Resource types without defining shared editor mechanics
```

A future editor framework may implement the Web editing experience, but it must
not define canonical Composition semantics through its internal node or state
types. Composition must first own the editor-neutral document model and runtime
validation required by a real product slice.

### Decision rationale

The current structured creator interface is sufficient for every implemented
Phase 8 slice. Adding a rich editor now would create a framework-shaped content
model before AI World has defined the required document semantics.

The Rich Editor Gate therefore follows the accepted Technology Strategy:

```text
defer editor selection until Composition / CMS requirements are concrete.
```

Decision:

```text
keep the current typed Text Block and ordered composition model;

defer rich editor technology.
```

### Future activation gate

Rich editor technology is not rejected permanently. Re-evaluate when an
implemented creator slice requires a concrete editor-neutral capability such as
one or more of:

```text
multiple semantic content nodes within one Block;

inline marks with canonical validation;

nested lists or structured tables;

controlled inline Media or Knowledge embeds;

stable portable document serialization;

schema migrations for persisted rich content;

accessible selection-aware editing commands;

collaborative editing with defined conflict semantics;

editor extensions backed by owner-defined Block contracts.
```

Before choosing a framework, Composition must define the canonical
editor-neutral content requirements, security rules, persistence meaning, and
public contracts. The Web implementation may then evaluate technologies against
those accepted requirements.

### Decision result

```text
PHASE 8 RICH EDITOR GATE
CLOSED
...
The next required roadmap decision is the Phase 8 Localization Gate. It must
evaluate actual product rollout requirements for multiple locales and must not
introduce Localization Kernel or frontend localization technology for
hypothetical future languages.

The Localization Gate remains blocked until this roadmap-only Rich Editor Gate
...
## PHASE 9 ACTIVATION RECORD

Phase 9 — Engagement Platform is ACTIVE.

The activation occurs only after the final Phase 8 closure commit:

```text
f6c9181cd6aeeb8596239b623745ee6800077d22
docs(roadmap): close Phase 8
```
...
TAG OBJECT
e07e9ae9145b90bea3d804c5310f783d21640b29
...
The Phase 9 activation prerequisites are SATISFIED:

```text
User ownership
SATISFIED — the User Platform owns canonical User state and exposes deliberate
public contracts

Identity and Authorization
SATISFIED — shared Session, Role, and Permission semantics are implemented and
validated

Canonical Resource references
SATISFIED — Identifiers and the Knowledge, Media, and Composition owners expose
typed public reference boundaries

Multi-Universe reuse
SATISFIED — Devotional and Anime independently reuse shared Platform contracts

Phase 8 Composition / CMS
SATISFIED — Phase 8 is complete, green, tagged, and remotely verified
```

Phase 9 therefore starts with the minimum next real capability:
...
Canonical P9-M01 scope remains exactly:
...
```text
Engagement Platform
    owns Favorite semantics, validation, mutation, lifecycle, and persistence meaning

User Platform
    remains owner of canonical User state

Referenced Resource owner
    remains owner of the referenced Resource

Favorite
    references canonical User and Resource identifiers
    may retain bounded Universe context only where real requirements justify it
```

Favorite references do not copy or transfer ownership of the User or referenced
Resource. Engagement must consume deliberately exposed public contracts and must
not deep-import User, Knowledge, Media, Composition, or Universe internals.

P9-M01 must begin by inspecting the current repository and defining the minimum
typed Favorite Domain model, authorization boundary, and persistence semantics
required by a real cross-Universe consumer. This activation does not pre-design
its implementation.

The following later Phase 9 capabilities remain demand-driven and are NOT
activated merely because Phase 9 begins:

```text
P9-M02 Collections
    introduce where product requirements justify Resource collections

P9-M03 Activity History
    introduce only with product value, privacy, retention, and preference design

P9-M04 Progress
    introduce only for learning paths, Experiences, or long-running journeys

P9-M05 Notifications
    introduce only when product workflows require user-facing Notification semantics

P9-M06 Personalization
    begin only with explicit preferences or signals backed by requirements

Recommendation Gate
    remains closed until usage data, product value, privacy design, and evaluation exist

Community features
    comments, follows, ratings, and social graph remain optional
```

If Notifications are later justified, Engagement may own Notification semantics
while the Email Foundation remains owner of delivery mechanics. Phase 9
activation does not materialize either capability.

This activation intentionally introduces:

```text
no Engagement production package

no Favorite Domain model
...
no Notification model or Email integration
...
P9-M01 remains blocked until this roadmap-only activation commit itself completes
exact green `push/main` CI.
...
## P9-M01 FAVORITES CLOSURE RECORD

P9-M01 — Favorites is CLOSED.

The accepted implementation baseline is:

```text
20c53b36d9b868a875a9fc9db520ca1d3bf9ac7c
feat(engagement): add Resource Favorites
```

Parent:

```text
ea904adb39c412b471f7ac3f677b8ee5d58a8711
docs(roadmap): activate Phase 9
```
...
The implementation introduces the first Engagement production package:

```text
@ai-world/platform-engagement

public Domain/application surface
    Favorite
    FavoriteStore
    AddFavorite
    ListFavorites
...
controlled infrastructure surface
...
User Platform
    ...
    resolves the acting Actor to the canonical User through GetUserProfile

Referenced Resource owner
    retains ownership of the referenced Resource and its lifecycle

Identity & Access
    owns Session validation and the authenticated Actor identity
```

The closed Favorite Domain model is intentionally narrow:
...
`resourceId` is a stable generic Resource reference. Favorite does not copy,
transfer, or synchronously govern the referenced Resource's canonical state.
Consumers resolve target availability and lifecycle through the target owner's
public contract when presenting the Favorite.

This preserves an acyclic boundary: Engagement does not deep-import Knowledge,
Media, Composition, Devotional, or Anime internals and does not branch on a
named Universe.

The authenticated ownership flow is:
...
canonical User ID
    ↓
Engagement Favorite operation
```

Clients cannot choose `userId` or `actorId`. The REST response exposes only:
...
GET /engagement/favorites
    authenticated User-scoped list

DELETE /engagement/favorites/:resourceId
    204
    idempotent removal
```

Session validation occurs before transport or canonical Favorite validation.
The add request is a strict transport object, invalid canonical identifiers are
rejected, and client-supplied ownership fields are rejected.
...
UNIQUE (user_id, resource_id)
    one Favorite per User and Resource
    database-backed idempotent add

INDEX (user_id, created_at)
    bounded User-scoped listing
...
resource_id
    stable generic Resource identifier
    no ownership-transfer foreign key
```

The append-only migration is:

```text
20260822203000_engagement_favorite_baseline
```
...
The controlled test evidence verifies:

```text
Engagement unit suite
5 passed

Favorite PostgreSQL persistence integration
1 passed
...
```text
stable Resource references without copying target state;

canonical User and Resource identifier validation;

Actor-to-User ownership derivation;

User-scoped listing and removal isolation;

bounded listing;

idempotent add and removal;

Session-before-request-validation behavior;

rejection of client-supplied User ownership;

one Devotional scripture Resource and one Anime series Resource through the
same generic Favorite contract;

no userId or actorId disclosure in Favorite API responses.
```

Repository validation completed before the implementation commit included:
...
Prisma migration deploy and status against PostgreSQL
PASS — 27 migrations, database up to date

Database Foundation typecheck and lint
PASS

Engagement typecheck, lint, build, unit test, and integration test
PASS

API typecheck, lint, build, unit test, and integration test
PASS

architecture:check
PASS — production build and zero dependency violations
...
P9-M01 deliberately does NOT introduce:

```text
Favorite Web UI

Collection model or API
...
Personalization or recommendation infrastructure
...
The only dependency wiring added is the API application's workspace dependency
on the Engagement owner package. Provider SDKs and external packages are not
introduced.
...
```text
P9-M02 — Collections
PRODUCT-REQUIREMENT DECISION NEXT
```

Collections must not be implemented merely because P9-M01 is closed. P9-M02 may
activate only when current product requirements justify User-owned collections
of Resources and define the minimum required semantics.

This closure candidate changes documentation state only. It introduces no
runtime behavior, package dependency, lockfile, schema, migration, API, or
infrastructure change.

P9-M02 remains blocked until this exact documentation-closure commit completes
green `push/main` CI.
...
## P9-M02 COLLECTIONS PRODUCT-REQUIREMENT DECISION / ACTIVATION RECORD

P9-M02 — Collections is ACTIVATED for implementation.

The product-requirement decision is:

```text
SATISFIED
```

Accepted project evidence establishes a real User-facing requirement rather than
activating Collections merely because P9-M01 Favorites is closed.

The accepted Vision requires that a User should be able to:

```text
build collections
```

The canonical Capability Map already defines User Collections with:
...
and classifies the capability as:

```text
MEDIUM
```

The canonical Ownership Model establishes:
...
Referenced Resources
    remain owned by their canonical owners
```

Canonical terminology defines a Collection as:

```text
an intentional grouping of Resources
```
...
Collections when requirements justify them.

The current repository contains no Collection production model, API, repository,
schema, migration, or Web implementation. Activation therefore freezes only the
minimum implementation scope; it does not claim the capability is already built.

The minimum P9-M02 implementation scope is:

```text
User-owned Collection semantics

Engagement-owned Collection membership

generic Resource references without ownership transfer

create Collection

list/read the acting User's Collections as required for usable ownership

add Resource membership

remove Resource membership

strict canonical identifier and ownership validation

bounded User-scoped reads

PostgreSQL persistence

authenticated API vertical slice

Devotional + Anime generic Resource reuse proof
```

The implementation must preserve the existing authenticated ownership pattern:
...
canonical User
    ↓
Engagement-owned operation
```

Clients must not choose another User's ownership identity.

The implementation must not make Engagement the owner of referenced:
...
The following remain deliberately outside the minimum P9-M02 activation scope:

```text
public/shared Collection visibility
...
The Capability Map's existing `share Collection where allowed` possibility is not
pulled into the first implementation slice because the current requirement does
not yet freeze public visibility, collaboration, or sharing-policy semantics.

P9-M02 implementation must follow the governing rule:
...
This activation changes roadmap/governance state only.
...
The implementation milestone must remain evidence-driven and must not pull
P9-M03 Activity History or later Phase 9 capabilities forward.

---

## P9-M02 COLLECTIONS IMPLEMENTATION / CLOSURE RECORD

P9-M02 — Collections is CLOSED.

The accepted implementation commit is:

```text
0e3a9a44096a8e8c99df3d921290be7aa10f83c8
feat(engagement): add Resource Collections
```

Its parent is the accepted P9-M02 activation commit:

```text
308bf599e776d61da08ad25c754b7f9300c5a4b6
docs(roadmap): activate P9-M02 Collections
```

The exact implementation `push/main` CI evidence is:
...
The implementation closes the minimum accepted User Collection capability with
the public Engagement application surface:

```text
Collection
CollectionResourceMembership
CollectionStore
...
```text
Engagement Platform
    owns Collection semantics and persistence meaning
    owns Collection membership semantics and persistence meaning

User Platform
    owns canonical User state
    resolves Actor → User through GetUserProfile

Referenced Resource owners
    retain ownership and lifecycle of referenced Resources

Identity & Access
    owns Session validation and authenticated Actor identity
```
...
canonical User ID
    ↓
Engagement Collection operation
```

Clients cannot choose `userId` or `actorId`.

The closed Collection model is intentionally narrow:
...
Collection names are canonicalized and bounded:
...
User-scoped Collection reads are bounded:
...
Collection Resource membership reads are bounded:
...
The authenticated API vertical slice is:
...
POST /engagement/collections/:collectionId/resources
    body { resourceId }
    200
    idempotent add

GET /engagement/collections/:collectionId/resources
    authenticated owner-scoped membership list

DELETE /engagement/collections/:collectionId/resources/:resourceId
    204
    idempotent removal
```

Strict transport validation rejects ownership-field injection. Session resolution
occurs before Collection request parsing, and canonical identifiers are validated
inside the Engagement application boundary.

Persistence is owned by:

```text
engagement_collections
engagement_collection_resources
```

The main database invariants are:
...
engagement_collection_resources
    PRIMARY KEY (collection_id, resource_id)
        one membership per Collection + Resource
        database-backed idempotent add
    INDEX (collection_id, added_at)
    FOREIGN KEY collection_id → engagement_collections(id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
...
```text
20260822214500_engagement_collection_baseline
```
...
The implementation proof uses one Devotional Resource and one Anime Resource
through the same generic Collection membership contract. Engagement does not
branch on a named Universe and does not deep-import Universe-owned internals.

Controlled pre-commit validation completed successfully with:
...
isolated PostgreSQL migration deploy/status
PASS — 28 migrations, database up to date

database table / foreign-key / index verification
PASS

Database Foundation typecheck and lint
PASS

Engagement typecheck, lint, build, unit test, integration test
PASS

Collection API integration
PASS

root formatting, lint, typecheck, and unit tests
PASS

root integration
PASS — 22 files / 158 tests

browser E2E in repository CI mode
PASS — 8/8
...
architecture:check
PASS — zero dependency violations

git diff --check
PASS
```

The exact implementation commit then completed the canonical GitHub `push/main`
CI workflow successfully. The Validate job completed all required steps:
...
P9-M02 deliberately does NOT introduce:
...
The Capability Map's `share Collection where allowed` possibility remains deferred
until a real sharing/public-visibility requirement freezes those semantics.
...
```text
P9-M03 — Activity History
PRODUCT-VALUE DECISION NEXT
```

P9-M03 must not be implemented merely because Collections is closed. It may
activate only if current product value justifies tracking viewed/interacted
Resources and privacy, retention, and User-preference requirements are frozen.

This closure candidate changes documentation state only. It introduces no new
runtime behavior, package dependency, lockfile, schema, migration, API, Web, or
infrastructure change.

P9-M03 remains blocked until this exact P9-M02 closure documentation commit
completes green `push/main` CI.
...
## P9-M03 ACTIVITY HISTORY PRODUCT-VALUE DECISION RECORD

P9-M03 — Activity History is DEFERRED.
...
The accepted long-term Vision allows User history to contribute to future:
...
and the canonical Capability Map identifies potential Engagement behavior:
...
Those long-term capabilities do not establish a current implementation demand.

The P9-M03 roadmap gate explicitly requires:

```text
product value that justifies tracking viewed/interacted Resources
```

and the adjacent privacy requirement requires Activity History to respect:
...
```text
7bd572e29b4145969532c7a44080e35e4cd81995
docs(roadmap): close P9-M02 Collections
```
...
recently-viewed consumer

recent-interaction consumer
...
The controlled P9-M03 decision inspection found:

```text
Activity History production filename hits
0

Activity History production content hits
0

Personalization production file hits
0

Recommendation production file hits
0
```

Introducing Activity History now would therefore create privacy-sensitive tracking
and persistence before:

```text
a real product consumer exists;

retention semantics are frozen;

User preference/control semantics are frozen;

privacy behavior is frozen.
```
...
until a real product requirement establishes sufficient value and freezes the
minimum privacy, retention, and User-control semantics.

This decision introduces:

```text
no Activity History Domain model

no tracking API

no recently-viewed endpoint

no recently-interacted endpoint
...
P9-M03 may be reconsidered later when a real consumer exists.
...
P9-M04 must be evaluated against current demand for:
...
and must be deferred if those consumers do not yet exist.

This decision record changes roadmap/governance state only.
...
## P9-M04 PROGRESS PRODUCT-VALUE DECISION RECORD

P9-M04 — Progress is DEFERRED.
...
The canonical Capability Map identifies potential Progress semantics:
...
The P9-M04 roadmap gate permits implementation only if AI World adds:

```text
learning paths

Experiences that require completion/progress state
...
```text
42e1868e3572886b0c7d5a557de802e856d5a38a
docs(roadmap): defer P9-M03 Activity History
```

contains no concrete production Progress consumer.

The controlled P9-M04 decision inspection found:

```text
Progress / Journey / LearningPath production filename hits
0

strong Progress / Journey / completion consumer hits
0

weak Experience completion-tracking context hits
0

relevant Prisma Progress / Journey / LearningPath models
0
```

The inspection deliberately did not treat generic uses of the word:

```text
Experience
```

as sufficient activation evidence. P9-M04 requires actual persistent User
progress/completion semantics, not merely the existence of presentation or CMS
experiences.
...
Experience completion state

progress percentage

completion timestamps

resume-from-progress behavior
```

Introducing Progress now would therefore create User-scoped state and persistence
without a real completion-tracked consumer.
...
until a real learning path, completion-tracked Experience, or long-running
Journey establishes a concrete Product requirement.

This decision introduces:

```text
no Progress Domain model
...
no new infrastructure technology
```

P9-M04 may be reconsidered when a real completion-tracked consumer exists.
...
```text
P9-M05 — Notifications
PRODUCT-WORKFLOW DECISION NEXT
```

P9-M05 must be evaluated against current product workflows that require
user-facing Notification semantics. Email delivery availability alone must not
be treated as sufficient demand because Engagement owns Notification semantics
while Email Foundation owns delivery.
...
## P9-M05 NOTIFICATIONS PRODUCT-WORKFLOW DECISION RECORD

P9-M05 — Notifications is DEFERRED.

The product-workflow decision is:

```text
NOT YET JUSTIFIED
```

The canonical Notification capability may eventually include:
...
Engagement Platform
    owns
user-facing Notification state and semantics
...
The P9-M05 roadmap gate requires:

```text
a real product workflow that requires user-facing Notification semantics
```

Current repository evidence at:

```text
6cec64a153223bab74e9b8db893a9fc080e475ae
docs(roadmap): defer P9-M04 Progress
```

shows no concrete production Notification lifecycle.

The controlled P9-M05 decision inspection found:

```text
Notification / inbox production filename hits
0

strong Notification consumer / lifecycle hits
0

relevant Notification Prisma models
0

weak Email delivery hits
3

weak product alert/reminder language hits
18
```

The weak Email evidence consists of existing delivery/presentation behavior such as
email verification. It does not establish canonical Engagement-owned Notification
state.
...
presentation semantics do not establish:
...
delivery coordination lifecycle
```

There is currently no implemented product requirement for:
...
Introducing Notifications now would therefore create User-scoped state and
lifecycle semantics without a real product workflow consumer.
...
until a real product workflow requires user-facing Notification state.

This decision introduces:

```text
no Notification Domain model
...
no Notification inbox/feed

no read/unread lifecycle
...
no background Notification infrastructure

no new technology
```

Existing security-related email workflows remain owned by their business
capabilities and use the Email Foundation only for delivery.

P9-M05 may be reconsidered when a real product workflow requires canonical
...
P9-M06 must be evaluated against current explicit preference/signal demand.
Complex recommendation or ML infrastructure must not be introduced merely
because personalization is a long-term Vision capability.

This is the single end-of-task roadmap update for P9-M05.
...
## RECOMMENDATION GATE DECISION RECORD

The Phase 9 Recommendation Gate is CLOSED.

The decision is:

```text
KEEP_RECOMMENDATION_GATE_CLOSED
```
...
The gate is conjunctive. All four canonical requirements must exist together:
...
```text
acbd12d6f2a62a6238ee76017d6f9496c5253ff7
docs(roadmap): defer P9-M06 Personalization
```

does not satisfy any of those four requirements.

The controlled Recommendation Gate decision inspection found:

```text
Recommendation production filename hits
0

Recommendation production symbol / route / store / repository hits
0

Recommendation Prisma models
0

limited Engagement usage-context models
3
    Favorite
    Collection
    CollectionResource

accepted recommendation-specific usage-measurement evidence
0

accepted recommendation-specific product-value evidence
0

accepted recommendation-specific privacy-design evidence
0

accepted recommendation-specific evaluation-method evidence
0

generic recommendation/personalization privacy guardrail lines
5
```

The five generic privacy-related lines are long-term Vision, terminology, or
governance guardrails. They do not constitute an implementation-ready,
recommendation-specific privacy design.

The corrected decision inspection also deliberately distinguishes ordinary:

```text
map
```

text from the uppercase recommendation-ranking metric:

```text
MAP
```

so generic prose cannot satisfy the evaluation-method requirement.
...
state is useful Engagement context but does not prove that AI World has:

```text
sufficient recommendation-quality usage data
```

especially while Activity History and Personalization Signals remain deferred.
...
recommendation-specific experiments

recommendation-specific evaluation metrics
```

Introducing recommendation infrastructure now would therefore violate the
governing implementation rule:

> Build the minimum architecture necessary to support the next real capability,
> then validate it through working vertical slices before expanding further.

The Recommendation Gate remains closed until all four accepted conditions are
evidenced.

Reopening requires:

```text
sufficient real recommendation-quality usage data

a concrete product workflow demonstrating clear recommendation value

accepted recommendation-specific privacy/control design

an accepted evaluation method with measurable quality/product metrics
```

This decision introduces:

```text
no Recommendation Domain model

no recommender implementation
...
no ML recommendation infrastructure
...
```text
Community Features
PRODUCT-DEMAND DECISION NEXT
```

Community Features remain optional under the canonical roadmap and must not be
implemented unless current product demand justifies them.

This is the single end-of-task roadmap update for the Recommendation Gate.
...
## COMMUNITY FEATURES PRODUCT-DEMAND DECISION RECORD

Phase 9 Community Features are DEFERRED.

The product-demand decision is:

```text
DEFER_PHASE9_COMMUNITY_FEATURES
```

The canonical roadmap explicitly makes these features optional:
...
and states that they are not necessary to close Phase 9 unless the roadmap is
explicitly revised.

Current repository evidence at:

```text
ea90729c2e6d6769bddd81c1b50810990ce75123
docs(roadmap): close Phase 9 Recommendation Gate
```

shows no concrete Community feature consumer or lifecycle.

The controlled Community Features decision inspection found:

```text
Community production filename hits
0

strong Comment / Follow / Rating / SocialGraph consumer hits
0

relevant Community Prisma models
0

dedicated current product-demand hits
0
```
...
```text
Comment creation or moderation
...
Ordinary source-code comments and generic uses of words such as:

```text
rating
```

do not establish canonical Community feature semantics.

Introducing these optional capabilities now would create:
...
without a real product consumer.
...
These capabilities may be reconsidered when a concrete product workflow requires
them and their minimum semantics are frozen.

This decision introduces:

```text
no Comment Domain model

no Follow Domain model

no Rating Domain model

no Social Graph model

no Community persistence
...
This is the single end-of-task roadmap update for Community Features.
...
## PHASE 9 CLOSURE CRITERIA EVALUATION / FINAL CLOSURE RECORD

Phase 9 — Engagement Platform is COMPLETE.

The controlled final closure evaluation result is:

```text
PHASE9_CLOSURE_ELIGIBLE

5 / 5 CLOSURE CRITERIA
SATISFIED
```

The closure evaluation was performed against:

```text
334b52abb43c64231afa6fd4dc15be955c31d015
docs(roadmap): defer Phase 9 Community Features
```

with the accepted final Phase 9 production implementation remaining:
...
changed between that P9-M02 implementation commit and the final Phase 9 closure
evaluation baseline.

The accepted implemented core is:
...
Every conditional or optional Phase 9 capability has been explicitly resolved:
...
Recommendation Gate
CLOSED
PERSONALIZED RECOMMENDATION INFRASTRUCTURE DEFERRED
...
The five canonical closure criteria are satisfied as follows.

```text
1. uses shared Resource references
SATISFIED

Favorite.resourceId and CollectionResource.resourceId are generic canonical UUID
references. Engagement does not copy Resource ownership, create target-owner
foreign keys, or branch on Devotional or Anime in production code.
```

```text
2. works across Devotional and Anime
SATISFIED

The Favorite API integration proof creates real Devotional and Anime Knowledge
Resources and favorites both through the same generic Resource contract.

The Collection API integration proof creates real Devotional and Anime Knowledge
Resources and groups both through the same generic Collection membership contract.
```

```text
3. does not duplicate User ownership
SATISFIED

The canonical User remains owned by the User Platform.

Engagement persistence references the canonical users(id) relation.

Authenticated Engagement operations resolve:
Session
    → Actor
    → GetUserProfile
    → canonical User
...
Client transport cannot choose userId.
```
...
Favorites and Collections are scoped to the acting User.

Cross-User mutation/isolation behavior is explicitly tested.

Public Engagement responses omit userId and actorId ownership details.

Privacy-sensitive Activity History and Personalization Signals remain deferred,
and optional social/community state remains deferred instead of being collected
without accepted product/privacy requirements.
```

```text
5. uses canonical Authorization where required
SATISFIED

Engagement API controllers require canonical Identity & Access Session validation.

Only the authenticated Actor enters actor-facing Engagement operations.

Actor-to-User ownership is resolved through the User Platform public contract.

Cross-User mutation is rejected or isolated according to canonical ownership.
```

Final focused closure validation passed:

```text
Engagement unit tests
2 files
10 / 10 tests PASS

Engagement TypeScript
PASS
...
Engagement/API closure-proof Prettier
PASS

named-Universe Engagement production hits
0

production drift since P9-M02
0
...
Phase 9 final closure introduces:
...
The Phase 9 completion tag is deliberately NOT created by this closure candidate.

### Mandatory completion-tag postcondition

The exact final Phase 9 closure documentation commit produced from this candidate
must first receive its own successful:
...
The tag must peel to the exact green Phase 9 final closure commit.

Phase 10 does not begin merely because this candidate exists.

The next governance action after the exact final closure commit is green and the
remote annotated completion tag is independently verified is:
...
This is the single final Phase 9 closure roadmap/documentation update.
...
## P10-M02 FINAL CLOSURE RECORD

P10-M02 — Production Infrastructure Provider is CLOSED.

Accepted decision:

```text
e29d3ec69fa2f0e1ba3c330aa1accf542b95ea0e
docs(operations): select production infrastructure providers
```
...
The exact decision commit was accepted after its `push/main` CI gate was reported
green.

The initial production provider stack is:
...
The provider decision preserves AI World-owned boundaries:

```text
PostgreSQL / DATABASE_URL / Prisma
...
Provider-specific SDK types are not canonical AI World Domain, Platform,
Universe, API transport, Media, Knowledge, Discovery, or persistence contracts.

P10-M02 selects providers only.

It creates no:

```text
DigitalOcean production resource
DigitalOcean staging resource
provider account
database
Spaces bucket
Postmark server/domain
...
```text
P10-M03 owns Infrastructure as Code

P10-M04 owns controlled deployment and environment promotion

P10-M05 owns Metrics

P10-M06 keeps distributed tracing conditional on demonstrated runtime value

P10-M07 owns Alerting

P10-M08 must establish object-storage protection because Spaces availability
does not itself constitute AI World backup

P10-M10 must review runtime Secret handling during production security hardening

P10-M11 owns measured performance and later sizing evidence

P10-M13 owns final cost visibility
```

The accepted initial topology direction is:
...
DigitalOcean App Platform
  ...
  +-- API
        |
        +-- private DigitalOcean Managed PostgreSQL
        |
        +-- DigitalOcean Spaces
        |
        +-- Postmark SMTP
...
No public/internal routing mechanism, health gate, migration job, rollback
command, or staging-to-production promotion mechanism is selected by P10-M02.

Those belong to P10-M04.

The primary-cloud candidate set considered was:
...
DigitalOcean was selected for current AI World fit, not as a universal cloud
ranking.

The decision records provider exit paths by keeping canonical application
ownership above provider-specific infrastructure/configuration.
...
P10-M03 must evaluate and implement the minimum IaC strategy against this
accepted provider stack.

Terraform is an evidence-backed leading candidate because DigitalOcean and
Better Stack expose Terraform providers, but P10-M02 does not select a Terraform
version, state backend, module layout, or create any Terraform configuration.
...
The next canonical Phase 10 milestone remains:

```text
P10-M04 — Deployment Pipeline
```

Before P10-M04 begins, product evidence requires the interstitial Web Product
Readiness Gate defined below.
...
# 227A. Web Product Readiness Gate Before P10-M04

This interstitial gate is ESTABLISHED.

Accepted sequencing decision:

```text
7c30e696c8f37bd9530e0103062e7b51fa69f214
docs(web): establish product readiness gate
```

Accepted ADR:

```text
docs/14-decisions/adr/p10-web-product-readiness-gate.md
```

The exact decision commit was accepted after its `push/main` CI gate was
reported green.

The gate responds to direct local product evidence:
...
first-party Web product experience
FUNCTIONAL BUT NOT YET PRODUCT-READY
```

The gate does not renumber Phase 10 and does not cancel P10-M04.

Its purpose is to complete the existing first-party Web through bounded vertical
product slices before deployment-pipeline implementation begins.

Canonical Web Product Readiness sequence:
...
The accepted product architecture remains:
...
semantic HTML + semantic CSS custom-property design tokens
...
The durable product objective is not to freeze one visual style for the
lifetime of AI World.
...
reusable application-level primitives;
...
bounded Session startup/retry resilience;
...
WPR-M03 does not activate deferred:
...
WPR-M05 owns final local product-quality evidence:
...
Web Product Readiness engineering targets include:

```text
WCAG 2.2 Level AA
engineering target, not an unsupported legal conformance claim
...
The detailed product, accessibility, performance, Session resilience,
Engagement, public Experience, security, SEO, localization-readiness, and
non-goal rules are canonical in the accepted ADR rather than duplicated in full
here.

The gate introduces no:
...
with exact green closure evidence.

Local Product Readiness acceptance does not replace later staging acceptance.

WPR-M01 closure result:

```text
IMPLEMENTATION
3b062e6c055288c4c32ed2b185042986eac820c6
feat(web): establish product UI foundation

CI COMPATIBILITY CORRECTION
b55b078511b013175ea814c9e83d543798ea5ede
fix(web): restore session compatibility
...
```text
semantic Web design-token foundation;
responsive global Application shell;
application-local UI primitives;
AI World landing experience;
typed Devotional and Anime presentation mapping;
visible focus and reduced-motion baseline;
bounded Session retry UX;
desktop and 390px mobile browser shell proof;
full Session-security compatibility restoration;
full local browser E2E 9/9;
root build and architecture validation;
exact corrective commit green in GitHub CI.
```

WPR-M02 closure result:

```text
IMPLEMENTATION
549121d21c0d7d9dad59351df8dcaa8c25fdb4b0
feat(web): improve identity and account experience
...
Accepted WPR-M02 closure evidence includes:

```text
shared app-local Identity/Account presentation;
registration UX coherence;
sign-in UX coherence;
email-verification UX coherence;
password-recovery and reset UX coherence;
account/profile UX coherence;
authenticated and anonymous navigation preserved;
authorization-denied presentation preserved without changing server enforcement;
real SMTP-to-Mailpit verification and password-recovery proofs 2 / 2 PASS;
Web Vitest 41 tests PASS;
root integration validation 29 / 29 tasks PASS;
full Chromium browser E2E 16 / 16 PASS;
production build 22 / 22 tasks PASS;
architecture validation 659 modules / 2102 dependencies / 0 violations;
GitHub Actions CI / Validate GREEN.
```

WPR-M03 closure result:

```text
IMPLEMENTATION
e98751e49df79a052f98b10d0a863e2d1cb8131c
feat(web): deliver knowledge discovery and engagement
...
Accepted WPR-M03 closure evidence includes:

```text
public Knowledge browse/detail coherence;
existing Media presentation preserved through public Resource asset references;
global and Universe-scoped Search coherence;
authenticated Favorites Web flow;
authenticated Collections Web flow;
Saved workspace for account-owned Favorites and Collections;
Session-scoped Engagement state keyed to authenticated actor identity;
normal-user product journey from registration/sign-in through Resource detail save actions and Saved management;
responsive Explore, Search, Resource, and Saved states;
Devotional and Anime presentation through the shared Web experience;
deferred Activity History, Progress, Notifications, Personalization, recommendations, and Community features remain unactivated;
Web Vitest 43 / 43 PASS;
root integration validation 29 / 29 tasks PASS;
API integration 158 / 158 PASS;
focused WPR-M03 Chromium browser proof 3 / 3 PASS;
full Chromium browser E2E 18 / 18 PASS;
production build 22 / 22 tasks PASS;
architecture validation 665 modules / 2128 dependencies / 0 violations;
GitHub Actions CI / Validate GREEN.
```

WPR-M04 closure result:

```text
IMPLEMENTATION
90ded20ad37a563483e1591fb9c6865981ee9aee
feat(web): deliver creator and public experiences
...
Accepted WPR-M04 closure evidence includes:

```text
dedicated Composition-owned anonymous public Experience transport;
explicit public published-Experience response contract;
DRAFT and ARCHIVED Pages remain unavailable from the public route;
malformed public Page identifiers remain opaque;
non-published Knowledge references are suppressed from public Experience output;
typed Block, published Knowledge, and Media public rendering;
Universe-aware public Experience presentation;
Creator workspace integrated into the shared AI World product shell;
creator Page creation, ordered composition, saved preview, publish, public-result navigation, and archive flow preserved;
AI-generated Knowledge candidates remain visibly non-canonical until explicit acceptance;
Devotional public Experience proof with existing multi-Universe presentation contract preserved;
focused Web validation 8 / 8 PASS;
root test validation 37 / 37 tasks PASS;
Web Vitest 45 / 45 PASS;
focused Composition integration 8 / 8 PASS;
root integration validation 29 / 29 tasks PASS;
API integration 158 / 158 PASS;
focused WPR-M04 Chromium browser proof 4 / 4 PASS;
full Chromium browser E2E 19 / 19 PASS;
production build 22 / 22 tasks PASS;
architecture validation 670 modules / 2147 dependencies / 0 violations;
GitHub Actions CI / Validate GREEN.
```

The next delivery task is:
...
No separate WPR-M05 activation commit is required.
